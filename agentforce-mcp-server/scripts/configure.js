...
async function configureServer() {
  ...
  console.log(`${colors.blue}  AgentForce MCP Server Configuration Wizard  ${colors.reset}`);
  console.log(`${colors.blue}============================================${colors.reset}`);
  console.log('');
  ...
  const port = await prompt(`Server Port ${config.server?.port ? `[${config.server.port}]` : '[3000]'}: `);
  config.server = config.server || {};
  config.server.port = port ? parseInt(port, 10) : (config.server.port || 3000);
  ...
  // Names and versions
  config.server.name = config.server.name || 'agentforce-mcp-server';
  ...
  console.log('');
  console.log(`With HTTP server support (recommended):`);
